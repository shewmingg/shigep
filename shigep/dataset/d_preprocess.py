#from https://github.com/eladhoffer/convNet.pytorch/blob/master/data.py
import torch
...
img_num=
from i in range(img_num):
    img_name = os.listdir(root_dir+)[i]
    img = Image.open(os.path.join)
    img = np.asarray(img, dtype = 'float32')
    img = img/255
    img_size = img.size/3
    img1 = img.reshape(img_size,3)
    img1 = img.transpose(img1)
    img_cov = np.cov([img1[0],img1[1],img1[2]])
    lambda, p = np.linalg.eig(img.cov)
    add_